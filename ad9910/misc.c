...
// Выключить static reset фазы и включить воспроизведение из оперативной памяти (если память задействована)
// Не изменяет снимок регистров; предполагается, что там static reset всегда включен, а воспроизведение из памяти - выключено
// Биты будут применены при первом переключении профиля
